...
if(!$size) {
  $query = "SELECT * FROM itens INNER JOIN imagens ON itens.id = imagens.id_produto WHERE categoria = 'Chuteira' AND imagens.thumb = 1 $andBrand $brand $andSize $size $andColor $color $order";
} else {
  $query = "SELECT * FROM itens INNER JOIN imagens ON itens.id = imagens.id_produto INNER JOIN unidades ON itens.id = unidades.id_produto WHERE categoria = 'Chuteira' AND imagens.thumb = 1 $andBrand $brand $andSize $size $andColor $color $order";
}
